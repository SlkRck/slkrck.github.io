...
$script:AutogenBegin = '<!-- BEGIN:AUTOGEN -->'
$script:AutogenEnd   = '<!-- END:AUTOGEN -->'
...
function Get-RepoRoot {
    ...
    $cur = if (Test-Path -LiteralPath $resolved -PathType Leaf) {
...
function Get-RelativePath {
    ...
    $toPath   = (Resolve-Path -LiteralPath $To).Path

    # Ensure "From" is treated as a directory URI
    $sep = [IO.Path]::DirectorySeparatorChar
    if (-not $fromPath.EndsWith($sep)) { $fromPath = $fromPath + $sep }

    $fromUri = [Uri]$fromPath
    $toUri   = [Uri]$toPath

    $rel = $fromUri.MakeRelativeUri($toUri).ToString()
    ($rel -replace '%20', ' ' -replace '\\', '/')
}
...
function Get-DirectoryIndexMarkdown {
    ...
    $items = Get-ChildItem -LiteralPath $DirectoryPath -Force -File -ErrorAction SilentlyContinue -Recurse:$Recurse |
        Where-Object {
    ...
    $groups = $items |
        ...
        ForEach-Object {
            ...
            [pscustomobject]@{
                Ext     = $_.Name
                SortKey = if ($idx -lt 0) { 999 } else { $idx }
                Group   = $_.Group
            }
        } |
        Sort-Object SortKey, Ext

    $md = New-Object System.Collections.Generic.List[string]
    $md.Add('### Indexed Files')
    $md.Add('')

    $recursionNote = if ($Recurse) { ' (recursive)' } else { '' }
    $md.Add("This section is generated. It lists files found in this directory$recursionNote.")
    ...
    foreach ($g in $groups) {
        $ext = if ([string]::IsNullOrWhiteSpace($g.Ext)) { '(no extension)' } else { $g.Ext }
        $md.Add("#### $ext")
        $md.Add('')

        foreach ($f in $g.Group) {
            # Use relative path if recursive to avoid ambiguous duplicates
            $display = if ($Recurse) {
                $rel = $f.FullName.Substring($DirectoryPath.Length).TrimStart('\', '/')
                ($rel -replace '\\', '/')
            }
            else {
                $f.Name
...
function Upsert-AutogenBlock {
    ...
    $begin = [regex]::Escape($script:AutogenBegin)
    $end   = [regex]::Escape($script:AutogenEnd)

    $rx = [regex]::new("(?s)($begin).*?($end)")

    if ($rx.IsMatch($ExistingText)) {
        return $rx.Replace($ExistingText, "`$1`n$GeneratedBlock`n`$2", 1)
    }
...
function Invoke-ReadmeGeneration {
    ...
    $targets = @(
        ...
        @{
            ...
            Bullets     = @('Modules/ – reusable Terraform modules and examples')
            IndexDir    = 'Terraform'
        },
        @{
